...
export default function UploadProgress({ fileName, progress, status, error, onRetry }: UploadProgressProps) {
  ...
  return (
    <div className="w-full max-w-md mx-auto">
      ...
      <div className="w-full bg-paper-dark rounded-full h-2.5 overflow-hidden">
        <div
          className={`h-full rounded-full transition-all duration-500 ease-out ${
            status === 'failed' ? 'bg-priority-high' : 'bg-accent'
          }`}
          style={{ width: `${percent}%` }}
        />
      </div>
      ...
      {/* 错误信息 + 重试 */}
      {status === 'failed' && (
        <div className="mt-4 rounded-lg bg-priority-high-bg border border-priority-high/20 p-4">
          <p className="text-sm text-priority-high mb-2">{error || '解析失败，请重试'}</p>
          {onRetry && (
            <button
              onClick={onRetry}
              className="text-sm text-priority-high font-medium underline hover:no-underline"
            >
              重新上传
            </button>
          )}
